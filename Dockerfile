...
\
### Optimize PHP5.6
	sed -i 's/^upload_max_filesize = 2M/upload_max_filesize = 120M/' /etc/php/5.6/apache2/php.ini && \
...
\
### Setup ODBC
	mv /tmp/odbcinst.ini /etc/odbcinst.ini && \
	mv /tmp/odbc.ini /etc/odbc.ini && \
\
### Install FreePBX 15.0.15.3
cd /usr/src && mkdir freepbx && \
	...
	fwconsole ma downloadinstall certman userman api pm2 &&\
